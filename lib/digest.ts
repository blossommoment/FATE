...
export type DigestPayload = {
  ...
  pages: { love: ReportPageText; career: ReportPageText; social: ReportPageText; season: ReportPageText };
};
...
export function buildDigestPrompt(facts: ReturnType<typeof buildPersonalFacts>): { system: string; user: string } {
  return {
    system: [
      "你是 FATE 深度解读报告的撰稿人。输入是 FATE 模型 2.0 已算好的个人事实清单（JSON），你负责把它写成四章报告正文。",
      "输出严格为 JSON（不要 markdown 代码块）：",
      `{"headline":"封面一句话人设，15字以内","pages":{"love":{"essay":"感情章评述，180~240字","advice":"给这个人的相处建议一条，40~70字"},"career":{"essay":"事业章评述，180~240字","advice":"落地行动建议一条，40~70字"},"social":{"essay":"人际章评述，160~220字","advice":"人际建议一条，40~70字"},"season":{"essay":"时运章评述，160~220字，可提及年龄段","advice":"当下策略一条，40~70字"}}}`,
      "铁律：",
      "1. 事实只能来自清单，不得新增判断或预测；每章评述围绕清单里该域的标签展开（love→tags.love，career→tags.career+recommendations，social→tags.social，season→currentPhase+environments）。",
...
export function validateDigestPayload(raw: unknown, _facts: ReturnType<typeof buildPersonalFacts>): DigestPayload | null {
  ...
  const keys = ["love", "career", "social", "season"] as const;
